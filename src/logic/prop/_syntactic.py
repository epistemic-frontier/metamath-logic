...
class Wi:
    ...
    def __call__(self, hphi: HypWff, hpsi: HypWff) -> Wff:
        if self.b is None:
            raise PreludeTypingError("wi: requires PropositionalBuiltins (Wi(b=...))")
...
class Mp:
    ...
    def __call__(self, hyp_phi: HypWff, hyp_imp: HypWff) -> Wff:
        if self.b is None:
            raise PreludeTypingError("mp: requires PropositionalBuiltins (Mp(b=...))")
...
class Wn:
    ...
    def __call__(self, hphi: HypWff) -> Wff:
        if self.b is None:
            raise PreludeTypingError("wn: requires PropositionalBuiltins (Wn(b=...))")
        require_hyp_sort_typed(hphi, "wff", ctx=self.label)
...
class Wa:
    ...
    def __call__(self, hphi: HypWff, hpsi: HypWff) -> Wff:
        if self.b is None:
            raise PreludeTypingError("wa: requires PropositionalBuiltins (Wa(b=...))")
        require_hyp_sort_typed(hphi, "wff", ctx=self.label)
        require_hyp_sort_typed(hpsi, "wff", ctx=self.label)
        return wa(self.b, hphi.body, hpsi.body)


def make_rules(b: PropositionalBuiltins) -> RuleBundle:
